...
export const buildPlatformDependentConfig = (params: {
  ...
  if (clientCertAuth) {
    const pfx = 'pfx' in clientCertAuth ? clientCertAuth.pfx : fs.readFileSync(clientCertAuth.pfxFilePath)
...
export const buildHeaders = (params: { userAgent?: string }) => {
  ...
  return {
    'User-Agent': `Node.js/${process.version}(${os.type()}) ${packageJson.name}@${packageJson.version}${
      userAgent ? ` ${userAgent}` : ''
    }`,
  }
}
